...
describe('session start registration validation', () => {
  ...
  it('rejects unknown careers and invalid legajo/DNI formats server-side', () => {
    ...
    expect(sessionStartSchema.safeParse({ ...base, identifierValue: '1234567' }).success).toBe(false)
    expect(sessionStartSchema.safeParse({ ...base, identifierValue: '1234567A' }).success).toBe(false)
