...
async function downloadBulkQRPdf(checkpoints: any[]) {
  const { generateBulkQRPdf } = await import('../lib/pdf')
  return generateBulkQRPdf(checkpoints)
}

async function downloadSingleQRPdf(checkpoint: any) {
  const { generateSingleQRPdf } = await import('../lib/pdf')
  return generateSingleQRPdf(checkpoint)
}
  ...
  return (
    <div className="min-h-screen bg-neutral-100 font-sans text-neutral-900 flex flex-col md:flex-row">
      ...
      <main className="min-w-0 flex-1 p-4 md:p-8 overflow-x-hidden">
        ...
        {activeTab === 'asistencia' && <AssistancePanel basePath="/api/admin/assistance" allowAlias />}
      </main>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => void downloadBulkQRPdf(checkpoints)} className="bg-blue-600 text-white px-4 py-2 rounded font-bold mr-2">
            Preparar Impresión
          </button>
    ...
    <div className="bg-white border rounded shadow-sm overflow-hidden">
      <div className="p-4 flex items-center justify-between bg-neutral-50 cursor-pointer hover:bg-neutral-100 transition-colors" onClick={onToggle}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2" onClick={e => e.stopPropagation()}>
          ...
          <button onClick={event => { event.stopPropagation(); void downloadSingleQRPdf(checkpoint); }} className="text-sm bg-blue-100 text-blue-700 px-3 py-1 rounded hover:bg-blue-200 font-medium">Descargar QR</button>
      ...
      {showQr && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label={`QR de ${checkpoint.label}`} onClick={() => setShowQr(false)}>
        <div className="w-full max-w-sm rounded-lg bg-white p-5 text-center shadow-xl" onClick={event => event.stopPropagation()}>
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-3"><button onClick={() => void downloadSingleQRPdf(checkpoint)} className="rounded bg-blue-600 px-4 py-2 text-sm font-bold text-white">Descargar QR</button><button type="button" onClick={() => setShowQr(false)} className="rounded border px-4 py-2 text-sm font-bold">Cerrar</button></div>
        </div>
      </div>}
    </div>
  )
